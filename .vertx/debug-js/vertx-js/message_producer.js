...
var MessageProducer = function(j_val) {
  ...
  this.exceptionHandler = function(handler) {
    ...
    if (__args.length === 1 && typeof __args[0] === 'function') {
      j_messageProducer.exceptionHandler(function(jVal) {
  ...
  this.write = function(data) {
    ...
    if (__args.length === 1 && true) {
      j_messageProducer.write(utils.convParamTypeUnknown(data));
      return that;
    } else utils.invalidArgs();
  };
  ...
  this.deliveryOptions = function(options) {
    ...
    if (__args.length === 1 && typeof __args[0] === 'object') {
      j_messageProducer.deliveryOptions(options != null ? new DeliveryOptions(new JsonObject(JSON.stringify(options))) : null);
      return that;
    } else utils.invalidArgs();
  };
